...
export function parseZoneConfigurations (): Zone[] {
  ...
  const environmentKeys = Object.keys(process.env).filter(key => key.startsWith('ZONE_'))
  ...
  return [...keys]
    .map(parseZoneConfig.bind(null, zoneOutputPath))
}
 ...
 */
function parseZoneConfig (zoneOutputPath: string, key: string): Zone {
  // Parse zone type first to determine what other fields are required
  const zoneType = z
    .enum(ZoneType)
    .default(ZoneType.Default)
    .parse(process.env[`ZONE_${key}_TYPE`])

  const conditionalShape: ZodRawShape = zoneType === ZoneType.Default
    ? {
        [`ZONE_${key}_INCLUDE`]: z
          .string()
    ...
    : {
        [`ZONE_${key}_PREFIX`]: z.string().regex(/^(\d{1,3}\.){1,3}$/)
      }

  // Abstract zone shape
  const shape: ZodRawShape = {
    [`ZONE_${key}_DOMAIN`]: RecordName,
    [`ZONE_${key}_IFACE`]: z.string(),
    [`ZONE_${key}_NS`]: CommaSeparatedRecordNames,
    [`ZONE_${key}_SOA_PRIMARY`]: RecordName,
    [`ZONE_${key}_SOA_RESPONSIBLE`]: RecordName,
    [`ZONE_${key}_STATIC_ONLY`]: z.string().optional().transform<boolean>(s => s === 'true'),
    [`ZONE_${key}_TYPE`]: z.enum(ZoneType).default(ZoneType.Default),
    RECORD_TTL: z.coerce.number().default(300),
    ...conditionalShape
  }
  ...
  if (zoneType === ZoneType.Default) {
    const fqdn = (result[`ZONE_${key}_DOMAIN`] as string).replace(/\.$/, '')
    return {
  ...
  const prefix = (result[`ZONE_${key}_PREFIX`] as string).replace(/\.$/, '')
  const fqdn = prefix.split('.').toReversed().join('.') + '.in-addr.arpa'
  return {
    domain: result[`ZONE_${key}_DOMAIN`],
    interface: result[`ZONE_${key}_IFACE`],
    key,
